...
import com.lantone.icss.api.at.model.yiqian.DrugDetailYiQian;
...
import com.lantone.icss.trans.yiqian.model.bean.DrugDetail;
import com.lantone.icss.trans.yiqian.model.bean.DrugInfo;
...
import com.lantone.icss.trans.yiqian.model.brxx2005.request.PreindexRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.PreindexRowRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.PrescriptRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.PrescriptRowDrugRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.PrescriptRowRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.ProjectItem;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.ProjectRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.request.ProjectRowRequest;
import com.lantone.icss.trans.yiqian.model.brxx2005.response.BrxxResponseBody;
import com.lantone.icss.trans.yiqian.model.brxx2005.response.BrxxResponseSearch;
import com.lantone.icss.trans.yiqian.model.brxx2005.response.VisitedObject;
...
public class InquiryService {
	private static Logger logger = LoggerFactory.getLogger(InquiryService.class);

	/**
	 * @Description:保存接口(改进版本)
	 * @author:ztg
	 * @time:2017年5月18日 下午4:10:38
	 */
	public InquiryResult saveInquiryInfoNew(InquirySaveWrapper data) throws RemoteException, ParseException {
	...
	//数据格式转换
	private void dataFormatNew(InquirySaveWrapper data, Brxx2005Request req) {
		//1、病人基础信息
		InquiryInfoWrapper info = data.getInfo();
		PatientInfo p = data.getToPat();
		...
		//pat.setBrjzxh(1L);					//病人就诊序号 his会传值 TODO
		...
		//2、病历信息
		List<InquiryDetailWrapper> toHistoryList = data.getToHistory();
		if(CollectionUtils.isNotEmpty(toHistoryList)) { // 空值判断
		...
		//3、疾病诊断
		List<DiseaseInfoWrapper> toDiagnoseList = data.getToDiagnose();
		if(CollectionUtils.isNotEmpty(toHistoryList)) { // 空值判断
		...
		//4、化验器查
		List<HisYiQianLisPacsInfo> toRequestList = data.getToRequest();
		if(CollectionUtils.isNotEmpty(toRequestList)) {
		...
		//5、预检指标
		List<HisYiQianPreindexInfo> toPreindexList = data.getToPreindex();
		if(CollectionUtils.isNotEmpty(toPreindexList)) {
		...
		//6、处方
		List<DrugYiQian> toPrescriptList = data.getToPrescript();
		if(CollectionUtils.isNotEmpty(toPrescriptList)) {
			List<PrescriptRowRequest> prescriptRowRequest = new ArrayList<PrescriptRowRequest>();
			for(DrugYiQian bean : toPrescriptList) {
				...
				pr.setBrcfts(bean.getBrcfts() == null? "":bean.getBrcfts().toString());
				List<DrugDetailYiQian> details = bean.getDetail();
				if(CollectionUtils.isNotEmpty(details)) {
					for(DrugDetailYiQian detail: details) {
