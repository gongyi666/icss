...
public class InquiryController {
	...
	/**
	 * @Description:保存诊断信息(改进版)
	 * @author:ztg
	 * @time:2017年5月12日 下午12:04:50
	 */
	@RequestMapping(value = "/saveInquiryNew")
	@ResponseBody	
	public Response<InquiryResult> saveInquiryNew(@RequestBody InquirySaveWrapper data) throws Exception {
		Response<InquiryResult> response = new Response<InquiryResult>();
		response.start();
		try {
			// 调接口
			logger.info("------------诊疗信息保存接口------------");
			response.setData(inquiryService.saveInquiryInfoNew(data));
